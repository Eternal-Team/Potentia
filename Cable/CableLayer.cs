...
namespace Potentia.Cable
{
	public class CableLayer : ModLayer<Cable>
	{
		...
		public override void Draw(SpriteBatch spriteBatch)
		{
			...
			if (Main.LocalPlayer.GetHeldItem().modItem is BasicCable && !elements.ContainsKey(mouse)) DrawPreview(Main.spriteBatch, mouse, Main.LocalPlayer.GetHeldItem().type);
		...
		public void DrawPreview(SpriteBatch spriteBatch, Point16 mouse, int type)
		{
			...
			if (elements.ContainsKey(mouse.X - 1, mouse.Y) && elements[mouse.X - 1, mouse.Y].type == type && elements[mouse.X - 1, mouse.Y].connections[Left]) frameX += 18;
			if (elements.ContainsKey(mouse.X + 1, mouse.Y) && elements[mouse.X + 1, mouse.Y].type == type && elements[mouse.X + 1, mouse.Y].connections[Right]) frameX += 36;
			if (elements.ContainsKey(mouse.X, mouse.Y - 1) && elements[mouse.X, mouse.Y - 1].type == type && elements[mouse.X, mouse.Y - 1].connections[Up]) frameY += 18;
			if (elements.ContainsKey(mouse.X, mouse.Y + 1) && elements[mouse.X, mouse.Y + 1].type == type && elements[mouse.X, mouse.Y + 1].connections[Down]) frameY += 36;
			...
			foreach (Point16 side in Cable.sides)
			{
				if (elements.ContainsKey(mouse + side))
				{
					...
					if (mouse.X + 1 == mouse.X + side.X && elements[mouse.X + side.X, mouse.Y + side.Y].type == type && elements[mouse.X + side.X, mouse.Y + side.Y].connections[Left]) otherFrameX += 18;
					if (mouse.X - 1 == mouse.X + side.X && elements[mouse.X + side.X, mouse.Y + side.Y].type == type && elements[mouse.X + side.X, mouse.Y + side.Y].connections[Right]) otherFrameX += 36;
					if (mouse.Y + 1 == mouse.Y + side.Y && elements[mouse.X + side.X, mouse.Y + side.Y].type == type && elements[mouse.X + side.X, mouse.Y + side.Y].connections[Up]) otherFrameY += 18;
					if (mouse.Y - 1 == mouse.Y + side.Y && elements[mouse.X + side.X, mouse.Y + side.Y].type == type && elements[mouse.X + side.X, mouse.Y + side.Y].connections[Down]) otherFrameY += 36;
		...
		public override void Load(List<TagCompound> tags)
		{
			...
			//elements.internalDict = tag.GetList<Point16>("Keys").Zip(tag.GetList<Cable>("Values"), (x, y) => new KeyValuePair<Point16, Cable>(x, y)).ToDictionary(x => x.Key, x => x.Value);
		...
		public override void Place(Point16 mouse, Player player, int type)
		{
			if (elements != null && !elements.ContainsKey(mouse))
			{
				//if (player.inventory.Any(x => x.modItem is BasicCable))
				//{
				// Consumes item
				//int type = player.inventory.First(x => x.modItem is BasicCable).type;
